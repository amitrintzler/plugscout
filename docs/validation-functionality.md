...
| --- | --- | --- | --- | --- | --- |
| Discover Claude plugins, Claude connectors, Copilot extensions, Skills, and MCP servers from one place | `src/lib/validation/contracts.ts` (`CatalogKindSchema`), `config/registries.json` (live plugin and connector sources + fallback registries), `src/catalog/sync.ts` (kind normalization/count logging) | `tests/integration/functionality-claims.spec.ts` (`discovers all catalog kinds and supports filtering by kind`), `tests/unit/registry-config.spec.ts`, `tests/integration/cli-flow.spec.ts` (`filters recommendations by requested kinds`) | `catalog-sync.yml` publishes per-kind item counts in summary | pass | Removed dead plugin APIs and added active Anthropic sources (`claude.com/plugins`, `claude.com/connectors`) plus GitHub marketplace manifests while preserving fallback entries. |
